...
const createTouchTexture = () => {
  ...
  const trail: { x: number; y: number; vx: number; vy: number; force: number; age: number }[] = [];
...
const PixelBlast: React.FC<PixelBlastProps> = ({
  ...
  useEffect(() => {
    ...
    if (mustReinit) {
      ...
      if (!gl) return;
      const renderer = new THREE.WebGLRenderer({ canvas, context: gl as WebGL2RenderingContext, antialias, alpha: true });
      ...
      // Global listeners so ripples work even when container has pointer-events: none
      const globalPointerDown = (e: PointerEvent) => {
        const rect = renderer.domElement.getBoundingClientRect();
  ...
  return (
    <div ref={containerRef} className={`pixel-blast-container ${className ?? ''}`} style={style} aria-label="PixelBlast interactive background" />
  );
};
